...
#define string_buffer_addstring(buffer, str) string_buffer_addlstring(buffer, str, 0);
#define string_buffer_addvalue(L, buffer)                                                       \
    do {                                                                                        \
        if(lua_isstring(L, -1))                                                                 \
            string_buffer_addlstring(buffer, lua_tostring(L, -1), luaL_len(L, -1));             \
        lua_pop(L, 1);                                                                          \
    } while(0)
